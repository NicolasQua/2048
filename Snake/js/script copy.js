...
export class SnakeStart {
    ...
    constructor() {
        ...
        let blockSize = 20; // set the size of the blocks (the size of the snake and the apple) as a global letiable
        let ctx; // set the context of the canvas (the color of the canvas) as a global letiable
        ...
        this.initCanvas = function () {
            ...
            requestAnimationFrame(animationLoop); // call the function (refresh the canvas)

            setInterval(() => {
                snakee.advance(); // call the function (advance the snake)
            }, 300); // call the function (advance the snake) with the delay as parameter
        };
        ...
        function animationLoop(time) {
            ...
            ctx.fillText("Score: " + score, blockSize, blockSize); // set the text (the score) with the position as parameters
            snakee.draw(ctx, blockSize); // call the function (draw the snake) with the context and the size of the blocks as parameters
            ...
            if (snakee.checkCollision(widthInBlocks, heightInBlocks)) { // if the snake is colliding with the wall or itself
                ...
                if (snakee.isEatingApple(applee)) { // if the snake is eating the apple
                    ...
                    do { // do while the apple is on the snake
                        applee.setNewPosition(widthInBlocks, heightInBlocks); // call the function (set the new position of the apple) with the width and the height of the canvas as parameters
        ...
        function refreshCanvas() {
            ...
            if (snakee.checkCollision(widthInBlocks, heightInBlocks)) { // if the snake is colliding with the wall or itself
                ...
                // timeout = setTimeout(requestAnimationFrame(refreshCanvas), delay); // call the function every 100ms (refresh the canvas) with the context and the delay as parameters
                requestAnimationFrame(refreshCanvas); // call the function (refresh the canvas)
            }
        }
